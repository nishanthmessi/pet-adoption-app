...
// @desc - Filter
// route GET /api/pet/location/${query}
// @access Public
const searchPetProfile = asyncHandler(async (req, res) => {
  const searchQuery = req.query.q
  const petProfiles = await Pet.find({ district: searchQuery })
  if (petProfiles) {
    res.status(200).json(petProfiles)
  } else {
    res.status(400)
    throw new Error('Unable to search profiles. Try again')
  }
})
